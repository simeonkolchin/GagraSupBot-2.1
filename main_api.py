...
start_message_text = f'<b>Приветствую! Я GAGRASUPbot</b> и мне хочется предложить вам отличный способ активно и ' \
                     f'увлекательно провести свой досуг - <b>сапбординг.</b> Это увлекательный вид активности, ' \
                     f'который понравится абсолютно любому - от взрослых до детей, любителям экстрима и просто ' \
                     f'желающим приятно провести время на море и не только.'\
                     f'\n\nМы предоставляем качественные и надежные сапборды от известных производителей, ' \
                     f'таких как Stormline, Bombitto и другие бренды. При этом, наш гид обязательно ' \
                     f'проведет <b>базовый инструктаж</b> перед началом сплава, а также предоставит ' \
                     f'водонепроницаемые чехлы для вашего телефона и спасательные жилеты, чтобы обеспечить вашу безопасность.'\
                     f'\n\nМы предлагаем различные варианты для всех желающих - прокат сапбордов, организованные ' \
                     f'прогулки и индивидуальные туры. Вам просто нужно <b>нажать на кнопку "Прайс"</b> и выбрать ' \
                     f'наиболее подходящий вариант для себя. При этом, если погода не будет благоприятной, мы всегда ' \
                     f'готовы перенести ваши прогулки на более удобное время.'\
                     f'\n\nНе упускайте возможность открыть для себя новый увлекательный ' \
                     f'мир морских прогулок - выберите сапбординг с GAGRASUP!'
...
def start(message):
    if message.chat.id in admins:
        ...
        keyboard = types.InlineKeyboardMarkup(row_width=1)
        keyboard.add(*buttons)
        bot.send_message(message.chat.id, start_message_text, parse_mode='html', reply_markup=keyboard)
...
def callback_inline(call):
    ...
    if call.data == 'ClbStart':
        if call.message.chat.id in admins:
            ...
            keyboard.add(*buttons)
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text=start_message_text, parse_mode='html', reply_markup=keyboard)
            dbworker.set_state(call.message.chat.id, config.States.S_START.value)
    ...
    if call.data == "ClbBot-A":
        ...
        keyboard.add(*buttons)
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text=start_message_text, parse_mode='html',
                              reply_markup=keyboard)
